...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['save'])) {
    ...
    if (empty($question) || empty($answer) || empty($difficulty)) {
        ...
        // Insert into DB
        $stmt = $conn->prepare("INSERT INTO flashcards (user_id, subject, notes, difficulty, question, answer, image_path, pdf_path, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");

        if ($stmt) {
            $stmt->bind_param("isssssss", $user_id, $subject, $notes, $difficulty, $question, $answer, $imagePath, $pdfPath);
            if ($stmt->execute()) {
                $stmt->close();
                $_SESSION['success'] = "";
